...
describe Guard::Gospel::Runner do
  ...
  describe '#run' do
    ...
    let(:out) { Guard::Gospel::Writer.new([default_options[:basename], default_options[:tempdir]])}
    ...
    context 'when fail command' do
      it "return 1" do
        ...
        allow(out).to receive(:string) {"\\e[31m\\e[1m.\\e[0m\\e[31m\\e[1mF\\e[0m\ndummy\n"}
        expect(Guard::Gospel::Writer).to receive(:new).with(['go_spel', '.tmp']) {out}
      ...
      it "return 1 failed to 3" do
        allow(proc).to receive(:exit_code) {1}
        allow(out).to receive(:string) {"\\e[31m\\e[1m.\\e[0m\\e[31m\\e[1mF\\e[0m\\e[31m\\e[1mF\\e[0m\\e[31m\\e[1mF\\e[0m\\e[31m\\e[1m.\\e[0m\ndummy\n"}
        expect(Guard::Gospel::Writer).to receive(:new).with(['go_spel', '.tmp']) {out}
